...
public abstract class AbstractLogic extends AbstractBeanMapperSupport {
  ...
  protected <T extends AbstractEto, K extends ApplicationEntity> Page<T> mapPaginatedEntityList(Page<K> page,
      Class<T> castTo) {

    List<T> mappedEntities = getBeanMapper().mapList(page.getContent(), castTo);

    return new PageImpl<>(mappedEntities, page.getPageable(), mappedEntities.size());
  }
